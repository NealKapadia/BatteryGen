...
Run:  python -m pytest tests/ -q          (from the molforge/ folder)
"""
import importlib
import json
import subprocess
...
PARENT = ROOT.parent                                    # dir where `molforge` is importable
...
def test_public_api_import():
    from molforge.core import config, data, model, infer, finetune, api  # noqa: F401
...
LOCAL_MODULES = sorted(f"molforge.{sub}.{m}" for sub, mods in _SUBMODULES.items() for m in mods)
...
def test_descriptors():
    from molforge.core import config, data
...
def test_finetune_build_dataset(vocab):
    from molforge.core import config, finetune
...
def test_finetune_rejects_junk(vocab):
    from molforge.core import finetune
...
def test_target_presets_switch_framing():
    from molforge.predictive.target import PRESETS
...
def test_feature_order_responds_to_config():
    from molforge.predictive import features
...
def test_transform_roundtrip(monkeypatch):
    import molforge.predictive.transform as T
    from molforge.predictive import target
...
def test_predictive_pipeline_end_to_end(tmp_path):
    ...
    env = dict(**{k: v for k, v in _os_environ().items()},
               MOLVAE_ART_DIR=str(tmp_path / "art"), MOLVAE_CE_CSV=str(csv))

    def run(mod, *a):
        r = subprocess.run([sys.executable, "-m", f"molforge.predictive.{mod}", *a],
    ...
    code = (
        "import joblib; from molforge.predictive import train;"
...
def test_molforge_encode_decode_and_props():
    ...
    assert isinstance(mf.decode(z), str) and mf.decode(z)
...
def test_finetune_cli_end_to_end(tmp_path):
    ...
    out = tmp_path / "ft.pt"
    r = subprocess.run(
        [sys.executable, "-m", "molforge.core.finetune", "--input", str(smi),
