"""Central configuration for the BatteryGen SELFIES-VAE pipeline.
...
(all prefixed ``BATTERYGEN_``; see ``.env.example``) or by editing this file.

Modules import this by its package-absolute path
(``from batterygen.core import config``), so every entry point runs as
``python -m batterygen.<subpackage>.<module>`` with no ``sys.path`` juggling.
"""
...
# config.py lives in <DATA_DIR>/batterygen/core/, so DATA_DIR is three parents up.
DATA_DIR = Path(os.getenv("BATTERYGEN_DATA_DIR", str(Path(__file__).resolve().parents[2])))
SMILES_DIR = DATA_DIR / "SMILES"
SMILES_GLOB = "iis_smiles-*.txt.gz"

# All generated artifacts live here, separate from raw data + code.
ART_DIR = Path(os.getenv("BATTERYGEN_ART_DIR", str(DATA_DIR / "batterygen_artifacts")))
...
# find them automatically. Override the folder with $BATTERYGEN_USER_DATA_DIR, point at
# a specific file with $BATTERYGEN_CE_CSV, or pass --csv (highest priority).
USER_DATA_DIR = Path(os.getenv("BATTERYGEN_USER_DATA_DIR", "data"))
...
def resolve_ce_csv(explicit: str | None = None) -> Path:
    ...
    env = os.getenv("BATTERYGEN_CE_CSV")
    if env:
        p = Path(env)
        if not p.exists():
            raise SystemExit(f"$BATTERYGEN_CE_CSV points to a missing file: {p}")
        return p
    ...
    if USER_DATA_DIR.is_dir():
        ...
        if len(csvs) > 1:
            ...
            raise SystemExit(
                ...
                f"Choose one with  --csv <file>  or set $BATTERYGEN_CE_CSV."
            )
    ...
    raise SystemExit(
        ...
        "  - set the $BATTERYGEN_CE_CSV environment variable\n"
...
def resolve_ce_csv_optional(explicit: str | None = None) -> Path | None:
    ...
    """
    if explicit or os.getenv("BATTERYGEN_CE_CSV"):
...
# --------------------------------------------------------------------------- #
SEED = int(os.getenv("BATTERYGEN_SEED", "42"))
...
N_WORKERS = int(os.getenv("BATTERYGEN_WORKERS", str(max(1, (os.cpu_count() or 4) - 1))))

# --------------------------------------------------------------------------- #
# Model hyper-parameters.
# "big" profile (default) is sized to use the RTX 3060 6 GB properly (~45M params)
# for an 8-10 h, higher-quality run. Set BATTERYGEN_PROFILE=small for the fast 4M model.
# --------------------------------------------------------------------------- #
_PROFILE = os.getenv("BATTERYGEN_PROFILE", "big").lower()
if _PROFILE == "small":
...
EMB_DIM = int(os.getenv("BATTERYGEN_EMB", EMB_DIM))
ENC_HIDDEN = int(os.getenv("BATTERYGEN_ENC_HIDDEN", ENC_HIDDEN))
DEC_HIDDEN = int(os.getenv("BATTERYGEN_DEC_HIDDEN", DEC_HIDDEN))
LATENT_DIM = int(os.getenv("BATTERYGEN_LATENT", LATENT_DIM))
ENC_LAYERS = int(os.getenv("BATTERYGEN_ENC_LAYERS", ENC_LAYERS))
DEC_LAYERS = int(os.getenv("BATTERYGEN_DEC_LAYERS", DEC_LAYERS))
DROPOUT = float(os.getenv("BATTERYGEN_DROPOUT", "0.2"))
...
WORD_DROPOUT = float(os.getenv("BATTERYGEN_WORD_DROPOUT", "0.25"))
...
BATCH_SIZE = int(os.getenv("BATTERYGEN_BATCH", "320"))
LR = float(os.getenv("BATTERYGEN_LR", "5e-4"))
LR_MIN = float(os.getenv("BATTERYGEN_LR_MIN", "2e-5"))
LR_WARMUP_MOLECULES = int(os.getenv("BATTERYGEN_LR_WARMUP", "1000000"))
GRAD_CLIP = 5.0
...
EPOCHS = int(os.getenv("BATTERYGEN_EPOCHS", "6"))
CHECKPOINT_EVERY = int(os.getenv("BATTERYGEN_CKPT_EVERY", "500000"))  # molecules seen
VAL_FRACTION = 0.01
# DataLoader workers. Default 0 on Windows (spawn overhead; data is already an
# in-RAM/memmap int array so workers buy little).
NUM_WORKERS = int(os.getenv("BATTERYGEN_NUM_WORKERS", "0"))
USE_AMP = os.getenv("BATTERYGEN_AMP", "1") != "0"
...
KL_WEIGHT_MAX = float(os.getenv("BATTERYGEN_KL_MAX", "0.1"))
KL_CYCLES = int(os.getenv("BATTERYGEN_KL_CYCLES", "1"))
KL_RATIO = float(os.getenv("BATTERYGEN_KL_RATIO", "0.4"))
KL_ANNEAL_MOLECULES = int(os.getenv("BATTERYGEN_KL_ANNEAL", "2000000"))  # used only in small profile
PROP_LOSS_WEIGHT = float(os.getenv("BATTERYGEN_PROP_W", "1.0"))
FREE_BITS = float(os.getenv("BATTERYGEN_FREE_BITS", "0.03"))  # nats/dim, anti-collapse
